...
class GUIInfo {
    ...
    /**
     * @REQUIRES: 0<=src<6400;0<=dst<6400;
     * @MODIFIES: System.out;
     * @EFFECTS: \result == minFlowShortestPath;
     *          对整个地图进行广度优先搜索，获得src到dst之间的最短路径同时总流量最小，储存在minFlowShortestPath中
     *          若地图不连通则程序输出相关信息并退出
     * @THREAD_REQUIRES: None;
     * @THREAD_EFFECTS: \locked(GUIInfo.graph);
     */
    public static Vector<Point> SPFA(int src, int dst) {
        if (src == dst) {
            return null;
        }

        boolean[] inQueue = new boolean[6400];// inQ[i]表示点i是否在队列中
        int[] distance = new int[6400];// 距离数组,表示点i到起点的距离
        long[] flow = new long[6400];// 流量数组,表示点i到起点的流量之和
        int[] nextPoint = new int[6400];// next[i]表示点i的下一个点
        ...
        synchronized (GUIInfo.graph) {
            ...
            while (!queue.isEmpty()) {
                ...
                for (int i = 1; i <= 4; i++) {
                    ...
                    if (next < 0 || next >= 6400 || graph[now][next] != 1) {
                        continue;
                    }
                    distTemp = distance[now] + 1;
                    flowTemp = flow[now] + (long) GUIGV.getFlow(now / 80, now % 80, next / 80, next % 80);
     ...
     */
    public synchronized static Vector<Point> getShortestPath(Point srcPoint, Point dstPoint) {
        ...
        Vector<Point> minFlowShortestPath = SPFA(src, dst);
        return minFlowShortestPath;
     ...
     * @REQUIRES: srcPoint!= null && 0=<srcPoint.x<80 && 0=<srcPoint.y<80;dstPoint!=null && 0=<dstPoint.x<80 && 0=<dstPoint.y<80;
     * @MODIFIES: System.out;
     * @EFFECTS: \result == n.depth + 1;
     *           对整个地图进行广度优先搜索，获得src到dst之间的最短路径长度，储存在n.depth中
     *           若地图不连通则程序输出相关信息并退出
     * @THREAD_REQUIRES: None;
     * @THREAD_EFFECTS: \locked();\locked(GUIInfo.graph);
     */
    public static synchronized int getDistance(Point srcPoint, Point dstPoint) {
        if (srcPoint.equals(dstPoint)) {
            return 0;
        }
     ...
     * @MODIFIES: None;
     * @EFFECTS: \result == getDistance(new Point(x1, y1), new Point(x2, y2));
     *           对整个地图进行广度优先搜索，获得两点之间的最短路径长度
     */
    public int getDistance(int x1, int y1, int x2, int y2) {// 使用bfs计算两点之间的距离
...
class GUIGV implements Constant {
    ...
    /**
     * @REQUIRES: 0=<x1<80;0=<y1<80;0=<x2<80;0=<y2<80;0=<count<=Integer.MAX_VALUE;
     * @MODIFIES: GUIGV.memflowmap;
     * @EFFECTS: GUIGV.memflowmap.get(Key(x1, y1, x2, y2)) == count;
     *           GUIGV.memflowmap.get(Key(x2, y2, x1, y1)) == count;
     */
    public static void setFlow(int x1, int y1, int x2, int y2, int count) {// 设置一个道路流量
        synchronized (GUIGV.memflowmap) {
            GUIGV.memflowmap.put(Key(x1, y1, x2, y2), count);
            GUIGV.memflowmap.put(Key(x2, y2, x1, y1), count);
        }
    }
     ...
     * @EFFECTS: \result == flow;
     */
    public static int[] getFlow(Point p) {// 查询流量信息
        int[] flow = new int[4];
     ...
     * @REQUIRES: None;
     * @MODIFIES: GUIGV.memflowmap;
     * @EFFECTS: GUIGV.memflowmap.size()==0;
     * @THREAD_REQUIRES: None;
     * @THREAD_EFFECTS: \locked(GUIGV.memflowmap);
     */
    public static void clearMemFlow() {// 清空流量信息
        synchronized (GUIGV.memflowmap) {
            GUIGV.memflowmap = new HashMap<String, Integer>();
        }
    }
}
            ...
            for (int j = 0; j < 80; j++) {
                ...
                if (GUIGV.guiInfo.map[i][j] == 2 || GUIGV.guiInfo.map[i][j] == 3) {
                    ...
                    if (GUIGV.drawflow) {
                        ...
                        graphics2D.drawString("" + GUIGV.getFlow(i, j, i + 1, j), memj,
                ...
                if (GUIGV.guiInfo.map[i][j] == 1 || GUIGV.guiInfo.map[i][j] == 3) {
                    ...
                    if (GUIGV.drawflow) {
                        graphics2D.setColor(Color.BLUE);
                        graphics2D.drawString("" + GUIGV.getFlow(i, j, i, j + 1),
...
class TaxiGUI {// GUI接口类
    ...
    /**
     * @REQUIRES: \all int i,j;0=<i<MAP_SIZE,0=<j<MAP_SIZE;0=<map[i][j]<=3;size==MAP_SIZE;
     * @MODIFIES: GUIGV.guiInfo.map;GUIInfo.graph;
     * @EFFECTS: \all int i,j;0=<i<MAP_SIZE,0=<j<MAP_SIZE;map[i][j]==GUIGV.guiInfo.map[i][j];
     * @THREAD_REQUIRES: None;
     * @THREAD_EFFECTS: \locked(graph);
     */
    public void updateMap(int[][] map, int size) {
    ...
    public void RequestTaxi(Point src, Point dst) {
        ...
        // 计算最短路径的值,通过一个窗口弹出
        // int distance = GUIGV.guiInfo.getDistance(src.x, src.y, dst.x, dst.y);
        // debugform form1 = new debugform();
        // form1.text1.setText("从(" + src.x + "," + src.y + ")到(" + dst.x + "," + dst.y
        // + ")的最短路径长度是" + distance);
    }
