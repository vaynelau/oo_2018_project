...
public class Light implements Runnable, Constant {
     ...
     * @REQUIRES: \this!=null;
     * @MODIFIES: None;
     * @EFFECTS: \result == (lightMap != null && systemTime >= 0 && timeWindow>=500 && timeWindow<=1000);
     */
    public boolean repOK() {
        return lightMap != null && systemTime>=0 && timeWindow >= 500 && timeWindow <= 1000;
    }

    /**
     * @REQUIRES: \this!=null;filePath != null;lightMap != null;
     * @MODIFIES: \this.lightMap;System.out;
     * @EFFECTS: 红绿灯文件正确 ==>(\all int i,j;0<=i<MAP_SIZE,0<=j<MAP_SIZE;0<=lightMap[i][j]<=2);
     *           为非十字或丁字路口设置红绿灯==>System.out控制台输出提示信息;
     *           红绿灯文件不存在或错误==> System.out控制台输出提示信息,程序结束;
     * @THREAD_EFFECTS: \locked(\this.lightMap);
     */
                    ...
                    if (scan.hasNextLine()) {
                        ...
                        for (int j = 0; j < MAP_SIZE; j++) {
                            if (Integer.parseInt(strArray[j]) == 1) {
                                count = Main.mapInfo.connectedEdges(i, j);
                                if (count >= 3) {
    ...
    /**
     * @REQUIRES: systemTime>=0 && timeWindow >= 500 && timeWindow <= 1000;
     * @MODIFIES: None;
    ...
    /**
     * @REQUIRES: \this!=null;lightMap != null && systemTime >= 0 && timeWindow>=500 && timeWindow<=1000;
     * @MODIFIES: \this.systemTime;\this.lightMap;
