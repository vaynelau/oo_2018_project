...
public class InputHandler implements Runnable, Constant {
     ...
     * @REQUIRES: \this != null;
     * @MODIFIES: None;
     * @EFFECTS: \result == (requestQueue != null && count>=0);
     */
    public boolean repOK() {
        return requestQueue != null && count>=0;
    }
    
    
    /**
     * @REQUIRES: requestQueue != null;
     * @MODIFIES: \this.requestQueue;
     * @EFFECTS: \this.requestQueue == requestQueue;
     */
     ...
     * @MODIFIES:System.out;Main.mapInfo;Light.lightMap;MapInfo.flowMap;Main.taxiQueue;\this.requestQueue;GUIInfo.graph;GUIGV.guiInfo.map;
     * @EFFECTS: LoadFile文件正确 ==> 地图，红绿灯，流量，出租车，请求等信息被初始化 && \result==true;
     ...
     * @EFFECTS: LoadFile文件正确 ==>指定地图或默认地图文件被装入;
     *           LoadFile文件错误或有其他异常发生 ==>程序结束; 
     */
     ...
     * @EFFECTS: LoadFile文件正确 ==>红绿灯文件被装入，红绿灯线程启动;
     *           LoadFile文件错误或有其他异常发生 ==>程序结束; 
     */
     ...
     * @EFFECTS: LoadFile文件正确 ==>文件中指定道路流量初始值等于设定值，其余道路流量为0;
     *           LoadFile文件错误或有其他异常发生 ==>程序结束;
     */
     ...
     * @EFFECTS: LoadFile文件正确 ==>对应出租车被设置为文件中指定的状态、信用和位置信息，未说明的出租车位置随机状态空闲;
     *           LoadFile文件错误或有其他异常发生 ==>程序结束;
     */
    private void setTaxi(Scanner scan) {
        ...
        if ("#taxi".equals(str)) {
            ...
            for (int i = 0; i < TAXI_NUM; i++) {
                ...
                point = new Point(x, y);
                if(i < 30) {
                    taxi = new TraceableTaxi(i, point, WAIT, Main.mapInfo, Main.taxiGUI);
                }else {
            ...
            for (Taxi t : Main.taxiQueue) {
                num = t.getTaxiNum();
                new Thread(t).start();
                Main.taxiGUI.setTaxiStatus(num, t.getCurrentPoint(), t.getStatus());
                Main.taxiGUI.SetTaxiType(num, (num < 30) ? 1 : 0);
     ...
     * @MODIFIES: \this.requestQueue;System.out;
     * @EFFECTS: LoadFile文件正确 ==>文件中指定的请求被处理，如果中间内容为空，则不响应请求;
     *           LoadFile文件错误或有其他异常发生 ==>程序结束;
     */
     ...
     * @MODIFIES: \this.requestQueue;\this.count;System.out;
     ...
     * @EFFECTS: 对地图上原本已存在的道路进行合法操作 ==>对应道路的状态被更新;
    ...
    private void setRoad(String str) {
        ...
        if (status == 0 && Main.mapInfo.isConnectedNow(x1, y1, x2, y2)) {
            Main.taxiGUI.setRoadStatus(new Point(x1, y1), new Point(x2, y2), status);
            return;
        }
        if (status == 1 && !Main.mapInfo.isConnectedNow(x1, y1, x2, y2) && Main.mapInfo.isConnectedFirst(x1, y1, x2, y2)) {
            Main.taxiGUI.setRoadStatus(new Point(x1, y1), new Point(x2, y2), status);
            return;
        }
    ...
    /**
     * @REQUIRES: \this != null;Main.taxiQueue != null;count>=0;
     * @MODIFIES: System.out;
     * @EFFECTS: 未捕捉到异常 ==> 对控制台输入的信息根据不同的类型调用对应方法分类进行处理，对无效输入System.out输出提示;
     *           (捕捉到异常 || "END".equals(inputStr)) ==> 程序退出;
     */
    public void run() {
        ...
        try {
            ...
            while (true) {
                ...
                matcher = pSERVE_COUNT.matcher(inputStr);
                if(matcher.matches()) {
                    String[] strArray = inputStr.split("[\\(\\)]");
                    TraceableTaxi taxi = (TraceableTaxi) Main.taxiQueue.get(Integer.parseInt(strArray[1]));
                    System.out.println(taxi.serveCount());
                    continue;
                }

                matcher = pHAS_NEXT.matcher(inputStr);
                if(matcher.matches()) {
                    String[] strArray = inputStr.split("[\\(\\)]");
                    TraceableTaxi taxi = (TraceableTaxi) Main.taxiQueue.get(Integer.parseInt(strArray[1]));
                    System.out.println(taxi.hasNext());
                    continue;
                }
                
                matcher = pHAS_PREVIOUS.matcher(inputStr);
                if(matcher.matches()) {
                    String[] strArray = inputStr.split("[\\(\\)]");
                    TraceableTaxi taxi = (TraceableTaxi) Main.taxiQueue.get(Integer.parseInt(strArray[1]));
                    System.out.println(taxi.hasPrevious());
                    continue;
                }
                
                
                matcher = pNEXT.matcher(inputStr);
                if(matcher.matches()) {
                    String[] strArray = inputStr.split("[\\(\\)]");
                    TraceableTaxi taxi = (TraceableTaxi) Main.taxiQueue.get(Integer.parseInt(strArray[1]));
                    taxi.next();
                ...
                matcher = pPREVIOUS.matcher(inputStr);
                if(matcher.matches()) {
                    String[] strArray = inputStr.split("[\\(\\)]");
                    TraceableTaxi taxi = (TraceableTaxi) Main.taxiQueue.get(Integer.parseInt(strArray[1]));
                    taxi.previous();
