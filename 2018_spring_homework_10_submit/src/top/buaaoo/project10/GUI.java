...
class GUIInfo implements Constant {
    ...
    /**
     * @REQUIRES: \all int i,j;0<=i<MAP_SIZE&&0<=j<MAP_SIZE;0<=map[i][j]<=3;
     * @MODIFIES: graph;
     * @EFFECTS: \all int i,j;0<=i=j<6400;graph[i][j]==0; \all int i,j;0<=i<6400 &&
     *           0<=j<6400 && i!=j;graph[i][j]==graph[i][j] && (graph[i][j]==1 ||
     *           graph[i][j]==gv.MAXNUM);
     */
